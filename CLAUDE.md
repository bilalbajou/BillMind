...
5. **Job Step 3 — Anomaly Detection:** `AnomalyDetectorService::detect()` runs 5 coherence checks and saves boolean flags via `forceFill()`
6. Job saves `ocr_text` + extracted fields to `invoices`, then saves line items to `invoice_items`
...
| `new_supplier` | Supplier name never seen in any other processed invoice (case-insensitive) |
...
### Key Models
- `Invoice` — soft deletes, tenant global scope, 60+ fillable fields. Has both `category` (raw AI string) and `category_id` (FK to `InvoiceCategory`); also `category_corrected_id` for user overrides. Lifecycle fields (`status`, `error_message`) and anomaly flags are **excluded from `$fillable`** — always use `forceFill()` when writing them.
...
- `Tenant` — company profile; fields include `currency` (3-char ISO code, default `USD`), `industry`, `tax_id`, `logo`.
...
- `Dashboard.jsx` — stats (total invoices, revenue from processed invoices, pending count), category distribution table, last 5 processed invoices
...
- `GET /invoices/{id}` — JSON detail (invoice + items + categories + uploader)
